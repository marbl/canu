...
static char CM_ID[] = "$Id: TestAlign_CNS.c,v 1.1.1.1 2004-04-14 13:51:22 catmandew Exp $";
...
int main (int argc, char *argv[]) {
   ...
   if (!merge)   {  // test "consensus" program alignments (fragment x fragment)
     ...
     MergeMultiAligns( sequenceDB, global_fragStore, pos_va, 1, 0, DP_Compare);
     if ( local_aligner ) 
        MergeMultiAligns( sequenceDB, global_fragStore, pos_va, 1, 0, Local_Overlap_AS_forCNS);
   }
   return 0;
}
