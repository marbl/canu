 ...
 *
 * $Id: CountMessages.c,v 1.1.1.1 2004-04-14 13:52:14 catmandew Exp $
 *
 */
...
{ 
 ...
 while (reader(stdin,&pmesg) != EOF){
   ...
   sizes[pmesg->t] += currPos - prevPos;
   prevPos = currPos;
 }

 for(i = 0; i <= MAX_MESG; i++){
   if(counts[i] != 0){
     fprintf(stdout,"* %s %8d total size " F_SIZE_T "   avg size " F_SIZE_T "\n",
	     MessageTypeName[i], counts[i], sizes[i], sizes[i]/counts[i]);
   }
