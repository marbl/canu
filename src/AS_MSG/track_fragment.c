 ...
 *************************************************************************/
/* $Id: track_fragment.c,v 1.1.1.1 2004-04-14 13:52:16 catmandew Exp $ */
...
int main( int argc, char ** argv )
{
  ...
  {
    fprintf( stderr,
             "Usage: %s input_file ((-f list_file) | UID_list))\n", argv[0] );
    fprintf( stderr,"     output goes to stdout\n");
    return 1;
  }
  ...
  else
  {
    ...
    for( i = 0; i < num_uids - 1; i++ )
    {
      ...
      AppendToID_Array( frag_iids, a2UID( string ), 0 );
    }
    fgets( string, 1000, fp_list );
    AppendToID_Array( frag_iids, a2UID( string ), 1 );
  ...
  while( reader( fp_in, &gen ) != EOF )
  {
    ...
    switch( gen->t )
    {
      case MESG_IUM:
	{ 
	  IntUnitigMesg * ium = (IntUnitigMesg *) gen->m;
	  for (ifrg=0;ifrg<ium->num_frags;ifrg++) {
	    if( FindID_ArrayID( frag_iids, ium->f_list[ifrg].ident) != -1) {
	      fprintf(stdout,"unitig " F_IID " contains frag " F_IID "\n",
                      ium->iaccession,ium->f_list[ifrg].ident); 
	    }
	  }
	}
        break;
