...
{
  uint32 num_v = GetNumFragments(frags);
...
uint32
BG_getOppositeVertex(BubGraph_t bg, IntEdge_ID e, uint32 v)
{
  uint32 a = get_avx_edge(bg->e, e);
  if (a == v)
...
int
BG_vertexForward(BubGraph_t bg, uint32 v)
{
...
int
BG_Degree(BubGraph_t bg, uint32 v, uint16 flags)
{
  BG_E_Iter it;
  IntEdge_ID e;
  int result = 0;
...
int
BG_outDegree(BubGraph_t bg, uint32 v, uint16 flags)
{
  BG_E_Iter it;
  IntEdge_ID e;
  int result = 0;
...
int
BG_inDegree(BubGraph_t bg, uint32 v, uint16 flags)
{
  BG_E_Iter it;
  IntEdge_ID e;
  int result = 0;
...
void
BG_V_setFlag(BubGraph_t bg, uint32 v, uint16 flag)
{
  bg->vFlags[v] |= flag;
}
...
void
BG_V_clearFlag(BubGraph_t bg, uint32 v, uint16 flag)
{
  bg->vFlags[v] &= ~flag;
}
...
uint16
BG_V_isSetFlag(BubGraph_t bg, uint32 v, uint16 flag)
{
  assert(NULL != bg);
...
uint16
BG_V_getFlags(BubGraph_t bg, uint32 v)
{
  return bg->vFlags[v];
}
...
void
BG_V_setDistance(BubGraph_t bg, uint32 v, int64 p)
{
  bg->vPos[v] = p;
}
...
int64
BG_V_getDistance(BubGraph_t bg, uint32 v)
{
  return bg->vPos[v];
}
...
IntEdge_ID
BGEI_bgn(BubGraph_t bg, BG_E_Iter_t it, uint32 v, BG_E_IterType t,
	 uint16 flags)
{
