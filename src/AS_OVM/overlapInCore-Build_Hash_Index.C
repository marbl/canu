...
static const char *rcsid = "$Id: overlapInCore-Build_Hash_Index.C,v 1.1 2011-07-30 01:16:05 brianwalenz Exp $";
...
Put_String_In_Hash(int i) {
  ...
  int  skip_ct;
  //int  screen_sub;
  //int  screen_lo = INT_MAX;
  //int  screen_hi = INT_MAX;
  ...
  if  (//guido(int) (getStringRefOffset(ref)) <= screen_lo - Kmer_Len + WINDOW_SCREEN_OLAP &&
       ! key_is_bad)
    {
  ...
  while  ((* p) != '\0')
    {
      //guidoif  ((int) (getStringRefOffset(ref)) == screen_hi - 1 - WINDOW_SCREEN_OLAP)
      //  {
