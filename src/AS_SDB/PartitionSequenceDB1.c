 ...
 *************************************************************************/
static char CM_ID[] = "$Id: PartitionSequenceDB1.c,v 1.13 2007-04-16 17:36:36 brianwalenz Exp $";
...
void usage(void){
  ...
  fprintf(stderr,"* usage:  PartitionSDB1 <sdbname> <sdbRevision> <fragsPerPartition> <inputFileName>\n");
  exit(1);
}
...
int main(int argc, char **argv){
  ...
  fprintf(stderr,
          "* Contig Size   Partition0:" F_SIZE_T "     Process:" F_SIZE_T "\n",
	  partition0ContigSize, totalContigSize);
  fprintf(stderr,
          "* Processed %d unitigs    %d to partition 0 and %d to process\n",
	  totalUnitigs + partition0Unitigs,
          partition0Unitigs, totalUnitigs);
  fprintf(stderr,
          "* Processed %d fragments    %d to partition 0 and %d to process\n",
	  totalFragments + partition0Fragments,
          partition0Fragments, totalFragments);
  return 0;
}
