...
prefixEditDistance::Set_Left_Delta(char  *A, char *T,
  ...
  for (int32 k=e; k>0; k--) {
    ...
    //fprintf(stderr, "prefixEditDistance::Set_Left_Delta()-- k-1=%5d d=%5d from=%5d lastr=%5d - r=%5d s=%5d d=%5d\n",
    //        k-1, d, from, lastr,
  ...
  for (int32 ei=1; ei <= Edit_Space_Max; ei++) {
    ...
    for (int32 d = Left;  d <= Right;  d++) {
      ...
      //  A mismatch.
      {
        ...
        if ((aPos <= 0) && (tPos <= 0)) {  //  If negative, we have a pointer into valid sequence.  If not, this mismatch
...

      {
        ...
        //if (gapCost == 0)
        //  fprintf(stderr, "prefixEditDistance::reverse()--  free A gap for aPos=%d tPos=%d t=%c/%d\n", tPos + d, tPos, T[tPos], T[tPos]);
...

      {
        ...
        //if (gapCost == 0)
        //  fprintf(stderr, "prefixEditDistance::reverse()--  free T gap for aPos=%d tPos=%d t=%c/%d\n", aPos, aPos - d, A[aPos], A[aPos]);
...
#ifdef DEBUG
        fprintf(stderr, "prefixEditDistance::reverse()-- e=%d MIN=%d Tail_Len=%d Max_Score=%d Score=%d slope=%f SLOPE=%f\n",
...
#ifdef DEBUG
  fprintf(stderr, "prefixEditDistance::reverse()-- iterated over all errors, return best found\n");
#endif
