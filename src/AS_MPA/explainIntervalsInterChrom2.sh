...
###########################################################################
#
# $Id: explainIntervalsInterChrom2.sh,v 1.1.1.1 2004-04-14 13:51:59 catmandew Exp $
#
...
function ProcessFiles
{
  ...
  for file in `ls [0-9][0-9][0-9].txt`; do
    ...
    echo "Interval(start length), 10k, 2k, 50k, BacEnd, " >> ${outFile}
    numFields=`gawk '{if(m<NF)m=NF}END{print m}' ${IvFile}`
    paste -d ' ' ${IvFile} ${3}/${1}.${chr}.*.intervals.txt.counts | gawk -v f=${numFields} '{for(i=1;i<=f;i++){printf("%s ", $i)}printf(", ");for(i=f+1;i<=NF;i++)printf("%s, ", $i);printf("\n")}' >> ${outFile}
  done
}
...
cat *_all.csv ../unmapped/${TEMP_DIR}/*_all.csv | grep -v -i interval | sort -k 3n > allNoHeaderSorted.csv
