 ...
 *************************************************************************/
static char CM_ID[] = "$Id: Instrument_CGW.c,v 1.9 2006-08-24 13:53:57 ahalpern Exp $";
			    ...
			    int printtype)
{
  if(printtype==PRINTTABLE){
    ...
    {
      if(! USE_ALL_MATES && ! (USE_LONG_MATES && dptr->mu>15000))return;
    }
			     ...
			     int printType)
{
  ...
  
  {
    ...
    assert(dst!=NULL);
    if(! USE_ALL_MATES && ! (USE_LONG_MATES && dst->mu>15000))return;
  }
...
#define MARK_INTER_SCAFFOLD_MATES
    ...
    if(printMateUIDs){
      ...
      if(getGateKeeperFragmentStore(ScaffoldGraph->gkpStore.frgStore,fragIID,&gkpFrag)!=0){
	assert(0);
      }
      fragUID=gkpFrag.readUID;
      if(getGateKeeperFragmentStore(ScaffoldGraph->gkpStore.frgStore,mateIID,&gkpFrag)!=0){
	assert(0);
      }
