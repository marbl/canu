 ...
 *************************************************************************/
static const char CM_ID[] = "$Id: extendClearRanges.c,v 1.14 2005-12-12 22:05:37 brianwalenz Exp $";
...
int main( int argc, char *argv[])
{
  ...
  
  {
    ...
    if ((setPrefixName == FALSE) || (setFragStoreName == 0) || (setGatekeeperStore == 0)) {
      fprintf(stderr,"* argc = %d optind = %d setFragStoreName = %d setGatekeeperStore = %d outputPath = %s\n",
              argc, optind, setFragStoreName, setGatekeeperStore, outputPath);
  ...
  for (sid = scaffoldBegin; sid < scaffoldEnd; sid++) {
    ...
    while ( rcontigID != -1 ) {
      ...
      // another hack
      //if ((rcontig->id == 427007) && (gapNumber == 20))
      //  numLeftFrags = numRightFrags = 0;

      //  well, it skips this crash, and dies shortly after with the same problem.
      //if (lcontig->id == 427007)
      //  numLeftFrags = numRightFrags = 0;
              ...
              if (keepGap) { // the fragment extensions have succeeded
                ...
                if (lFragIid != -1) {
                  ...
                  if ( gotNewLeftMA ) {
                    ...
                    DumpContigMultiAlignInfo ( lcontig->id );
                    DumpContigMultiAlignInfoDirect ( new_cma, lcontig->id );
              ...
              if (keepGap)
                {
                  ...
                  fprintf(stderr, "CreateAContigInScaffold()-- newOffsetAEnd=%d nerOffsetBEnd=%d\n",
...
int extendCgwClearRange( int fragIid, int frag3pDelta)
{
  ...
  //  The OLD_WAY used surround this with a close(), open(), <the
  //  stuff below>, close(), open().  I guess to make sure that the
  //  change gets committed.  Anyway, it's gone now.
...
int revertToCnsClearRange( int fragIid )
{
  ...
  //  See the comment in extendCgwClearRange().
...
void updateIntUnitigPoss( NodeCGW_T *contig )
{
  ...
  MultiAlignT *new_cma = NULL;  //  BPW CreateEmptyMultiAlignT();
...
void getAlteredFragPositions( NodeCGW_T *unitig, fragPositions **fragPoss, int alteredFragIid, int extension)
{
  ...
  MultiAlignT *uma = NULL;  //  BPW CreateEmptyMultiAlignT();
