...
setClear(AS_IID IID, char *E, uint32 which, int update, int verbose) {
  ...
  if (verbose)
    fprintf(stdout, "frg iid "F_IID" %s %d %d -> %d %d\n",
...
edit_mateiid(AS_IID     IID,
  ...
  if (verbose)
    fprintf(stdout, "frg iid "F_IID" mateiid "F_IID" -> mateiid "F_IID"\n",
            fr.gkFragment_getReadIID(), o, fr.gkFragment_getMateIID());
  ...
  if ((n == 0) && (o != 0)) {
    ...
    if (verbose)
      fprintf(stdout, "frg iid "F_IID" mateiid "F_IID" -> mateiid "F_IID"\n",
              fr.gkFragment_getReadIID(), IID, fr.gkFragment_getMateIID());
...
edit_libiid(AS_IID     IID,
  ...
  if (verbose)
    fprintf(stdout, "frg iid "F_IID" libiid "F_IID" -> libiid "F_IID"\n",
            fr.gkFragment_getReadIID(), o, fr.gkFragment_getLibraryIID());
...
edit_isnonrandom(AS_IID     IID,
  ...
  if (verbose)
    fprintf(stdout, "frg iid "F_IID" isnonrandom "F_U32" -> "F_U32"\n",
            fr.gkFragment_getReadIID(), o, fr.gkFragment_getIsNonRandom());
...
edit_isdeleted(AS_IID     IID,
  ...
  if (verbose)
    fprintf(stdout, "frg iid "F_IID" isdeleted "F_U32" -> "F_U32"\n",
            fr.gkFragment_getReadIID(), o, fr.gkFragment_getIsDeleted());
...
edit_orientation(AS_IID     IID,
  ...
  if (verbose)
    fprintf(stdout, "frg iid "F_IID" orientation %s -> %s\n",
            fr.gkFragment_getReadIID(), AS_READ_ORIENT_NAMES[o], AS_READ_ORIENT_NAMES[fr.gkFragment_getOrientation()]);
...
editStore(char *editsFileName, char *gkpStoreName, int update) {
  ...
  while (!feof(F)) {
    ...
    if (IID == 0) {
      fprintf(stderr, "invalid id (IID="F_IID") in edit line: '%s'\n", IID, L);
      errors++;
      goto nextline;
    }
    ...
    if (isLIB) {
      ...
      if        (strcasecmp(ACT, "mean") == 0) {
        ...
        gklr.mean   = atof(E);
        if (verbose)
          fprintf(stdout, "lib iid "F_IID" mean %f -> %f\n",
                  IID, m, gklr.mean);
        ...
        gklr.stddev = atof(E);
        if (verbose)
          fprintf(stdout, "lib iid "F_IID" distance %f %f -> %f %f\n",
                  IID, m, s, gklr.mean, gklr.stddev);
...
#define setBoolean(XX)                                                  \
          ...
          if (verbose)                                                  \
            fprintf(stdout, "lib iid "F_IID" XX %c -> %c\n",                 \
                    IID,                   \
                    (o) ? 'T' : 'F',                                    \
                    (gklr.XX) ? 'T' : 'F');                             \
        ...
        if (verbose)
          fprintf(stdout, "lib iid "F_IID" orientation %s -> %s\n",
                  IID, AS_READ_ORIENT_NAMES[o], AS_READ_ORIENT_NAMES[gklr.orientation]);
