 ...
 *************************************************************************/
static char CM_ID[] 
= "$Id: cgi2fasta.c,v 1.1.1.1 2004-04-14 13:50:09 catmandew Exp $";
/* *******************************************************************
 *
 * Usage:
...
(int argc, char * argv [],
 ...
 const float      coverage_stat_threshold,
 const char       target[]
 )
{ /* It is assumed that in the overlap records that new fragments
  ...
  while( EOF != ReadMesg_AS(fcgi, &pmesg)) {
	...
	const int gapped_unitig_length = ium_mesg->length;
	char *chunk_sequence = ium_mesg->consensus;
        ...
        
	{
          ...
          int inew=0, iold;
          assert(gapped_unitig_length == gapped_len);
          assert(chunk_sequence[gapped_unitig_length] == '\0');
          for(iold = 0; iold < gapped_unitig_length; iold ++) {
            char ch = chunk_sequence[iold];
            if( ch != '-' ) {
              assert( ch == 'A' || ch == 'C' || ch == 'G' || ch == 'T' || ch == '\0' );
              chunk_sequence[inew] = chunk_sequence[iold];
              inew ++;
            }
            if( ch == '\0' ) break;
          }
          ungapped_unitig_length = inew;
          chunk_sequence[ungapped_unitig_length] = '\0';
        }

        if( (ium_mesg->coverage_stat >= coverage_stat_threshold) &&
            (ium_mesg->num_frags > 1) 
            ) { 
          fprintf(stdout,"> unitig=" F_IID " length=%dbp num_frags=" F_IID " Astat=%.2f\n",
                  cid, ungapped_unitig_length, num_frags, ium_mesg->coverage_stat );
          fprintf(stdout,"%s\n",chunk_sequence);
        }
#if 0
	for( ifrag=0; ifrag<num_frags; ifrag++) {
	  const IntFragment_ID iid = f_list[ifrag].ident;
	  const IntMultiPos imp = f_list[ifrag];
...
int main(int argc, char * argv [])
{
  ...
  float      coverage_stat_threshold = -10000000.;
  int illegal = FALSE;
  char *target = NULL;
