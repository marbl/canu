 ...
 *************************************************************************/
/*************************************************
* Module:  NewFragScriptOVL.c
* Description:
*   Create a batch script of LSF commands to compute overlaps
*   between the designated range of fragments in the specified frag
*   store and all the BACtigs in the specified BACtig store.  The range
*   is specified in the range-file named on the command line.
* 
*    Programmer:  A. Delcher
*       Written:  17 Feb 00
*  Last Revised:  17 Feb 00
* 
*************************************************/

/* RCS info
 * $Id: NewFragScriptOVL.c,v 1.3 2005-03-22 19:07:11 jason_miller Exp $
 * $Revision: 1.3 $
*/
...
#if  1
#define  NEW_BATCH_SIZE           500000
    //  Ideal number of new fragments to process in each LSF job
#define  OLD_BATCH_SIZE           2000
    //  Ideal number of old BACtigs to process in each LSF job
#else
#define  NEW_BATCH_SIZE           10000
    //  Ideal number of new fragments to process in each LSF job
#define  OLD_BATCH_SIZE           500
    //  Ideal number of old BACtigs to process in each LSF job
#endif
...
int  main  (int argc, char * argv [])

  {
   ...
   lsf_queue_name  = LSF_QUEUE_NAME;
   {
    ...
    while  (! errflg && ((ch = getopt (argc, argv, "L:Pq:s:t:")) != EOF))
      switch  (ch)
        {
         case  'L' :
   ...
   if  (existsFragStore (bactigstore_path))
       {
        ...
        old_bactig_ct = getLastElemFragStore (bactig_store);
   ...
   Emit_Script (scriptfile_name, lsf_queue_name,
                old_bactig_ct, first_frag, last_frag,
                fragstore_path, bactigstore_path, bol_path,
                option_string);


   printf ("old_bactig_ct = %d\n", old_bactig_ct);
...
//  to overlap fragments numbered  first_new .. last_new  in the
//  fragment store with all BACtigs in the BACtig store.
//  There are  old_ct  BACtigs in the BACtig store.
      ...
      for  (j = 0;  j < old_batches;  j ++)
        {
         ...
         fprintf (olap_script_fp, "%s -h %7d-%-7d -r %7d-%-7d %s"
                  " -b %s/" F_UID ".%d.bol %s %s\n",
                  OLAP_CMD, lo, hi, first, last, options,
   ...
   if  (tag_ct > 0)
       {
        ...
        fprintf (scriptfile,
                 "set bsubReturn = `bsub -K -q $QUEUE -P \"%s\""
                 " -J \"%s/%sSUCCESS\" -w \"done(${olapJobID}[1-%d])\""
