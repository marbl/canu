...
sub createOverlapStoreSequential ($$$@) {
    ...
    $cmd .= " -o $wrk/$asm.${typ}Store.BUILDING \\\n";
    $cmd .= " -g $wrk/$asm.gkpStore \\\n";
    $cmd .= " -M " . getGlobal("ovlStoreMemory") . " \\\n";
    $cmd .= " -L $files \\\n";
    $cmd .= " > $wrk/$asm.${typ}Store.err 2>&1";

    if (runCommand($wrk, $cmd)) {
        caFailure("failed to create the overlap store", "$wrk/$asm.${typ}Store.err");
    }
...
sub overlapStoreBucketizerCheck ($$$$$) {
    ...
    while (<F>) {
        ...
        if (! -e "$wrk/$asm.${typ}Store.BUILDING/bucket$bucketID") {
            $failureMessage .= "  job $wrk/$asm.${typ}Store.BUILDING/bucket$bucketID FAILED.\n";
    ...
    if ($attempt < 2) {
        submitOrRunParallelJob($wrk, $asm, "ovB", "$wrk/$asm.${typ}Store.BUILDING", "scripts/1-bucketize", getGlobal("osbConcurrency"), @failedJobs);
    } else {
...
sub overlapStoreSorterCheck ($$$$$) {
    ...
    #    $wrk/$asm.${typ}Store.BUILDING/$sortID
    #    $wrk/$asm.${typ}Store.BUILDING/$sortID.index
    #    $wrk/$asm.${typ}Store.BUILDING/$sortID.info
    #
    #  A crashed result has one file, if it crashes before output
    #    $wrk/$asm.${typ}Store.BUILDING/$sortID.ovs
    #
    #  On out of disk, the .info is missing.  It's the last thing created.
    #
    while ($currentJobID <= $numSlices) {

        if ((! -e "$wrk/$asm.${typ}Store.BUILDING/$sortID") ||
            (! -e "$wrk/$asm.${typ}Store.BUILDING/$sortID.info") ||
            (  -e "$wrk/$asm.${typ}Store.BUILDING/$sortID.ovs")) {
            $failureMessage .= "  job $wrk/$asm.${typ}Store.BUILDING/$sortID FAILED.\n";
            unlink "$wrk/$asm.${typ}Store.BUILDING/$sortID.ovs";
            push @failedJobs, $currentJobID;
        } else {
            push @successJobs, $currentJobID;
        }
    ...
    if ($attempt < 2) {
        submitOrRunParallelJob($wrk, $asm, "ovS", "$wrk/$asm.${typ}Store.BUILDING", "scripts/2-sort", getGlobal("ossConcurrency"), @failedJobs);
    } else {
...
sub createOverlapStoreParallel ($$$@) {
    ...
    system("mkdir -p $wrk/$asm.${typ}Store.BUILDING")                   if (! -d "$wrk/$asm.${typ}Store.BUILDING");
    system("mkdir -p $wrk/$asm.${typ}Store.BUILDING/scripts")           if (! -d "$wrk/$asm.${typ}Store.BUILDING/scripts");
    system("mkdir -p $wrk/$asm.${typ}Store.BUILDING/logs")              if (! -d "$wrk/$asm.${typ}Store.BUILDING/logs");

    #  Parallel jobs for bucketizing.  This should really be part of overlap computation itself.

    if (! -e "$wrk/$asm.${typ}Store.BUILDING/scripts/1-bucketize.sh") {
        open(F, "> $wrk/$asm.${typ}Store.BUILDING/scripts/1-bucketize.sh") or die;
        print F "#!/bin/sh\n";
        print F "\n";
        ...
        print F "fi\n";
        print F "\n";
        print F "if [ -e \"$wrk/$asm.${typ}Store.BUILDING/bucket\$bn/sliceSizes\" ] ; then\n";
        print F "  echo \"Bucket $wrk/$asm.${typ}Store.BUILDING/bucket\$bn finished successfully.\"\n";
        print F "  exit\n";
        print F "fi\n";
        print F "\n";
        print F "if [ -e \"$wrk/$asm.${typ}Store.BUILDING/create\$bn\" ] ; then\n";
        print F "  echo \"Removing incomplete bucket $wrk/$asm.${typ}Store.BUILDING/create\$bn\"\n";
        print F "  rm -rf \"$wrk/$asm.${typ}Store.BUILDING/create\$bn\"\n";
        print F "fi\n";
        print F "\n";
        print F getBinDirectoryShellCode();
        print F "\n";
        print F "\$bin/ovStoreBucketizer \\\n";
        print F "  -o $wrk/$asm.${typ}Store.BUILDING \\\n";
    ...
    if (! -e "$wrk/$asm.${typ}Store.BUILDING/scripts/2-sort.sh") {
        open(F, "> $wrk/$asm.${typ}Store.BUILDING/scripts/2-sort.sh") or die;
        print F "#!/bin/sh\n";
        print F "\n";
        print F "jobid=\$SGE_TASK_ID\n";
        print F "if [ x\$jobid = x -o x\$jobid = xundefined ]; then\n";
    ...
    if (! -e "$wrk/$asm.${typ}Store.BUILDING/scripts/3-index.sh") {
        open(F, "> $wrk/$asm.${typ}Store.BUILDING/scripts/3-index.sh") or die;
        print F "#!/bin/sh\n";
        print F "\n";
        print F getBinDirectoryShellCode();
        print F "\n";
    ...
    system("chmod +x $wrk/$asm.${typ}Store.BUILDING/scripts/1-bucketize.sh");
    system("chmod +x $wrk/$asm.${typ}Store.BUILDING/scripts/2-sort.sh");
    system("chmod +x $wrk/$asm.${typ}Store.BUILDING/scripts/3-index.sh");

    overlapStoreBucketizerCheck($wrk, $asm, "ovl", $files, 0);  #  Compute
    overlapStoreBucketizerCheck($wrk, $asm, "ovl", $files, 1);  #  Compute again, if needed
    overlapStoreBucketizerCheck($wrk, $asm, "ovl", $files, 2);  #  Fail, if needed

    overlapStoreSorterCheck($wrk, $asm, "ovl", $files, 0);
    overlapStoreSorterCheck($wrk, $asm, "ovl", $files, 1);
    overlapStoreSorterCheck($wrk, $asm, "ovl", $files, 2);

    if (runCommand("$wrk/$asm.${typ}Store.BUILDING", "scripts/3-index.sh")) {
...
sub createOverlapStore ($$$$) {
    ...
    goto alldone if (-d "$wrk/$asm.${typ}Store");
    ...
    createOverlapStoreSequential($wrk, $asm, $typ, "$path/ovljob.files")  if ($seq eq "sequential");
    createOverlapStoreParallel  ($wrk, $asm, $typ, "$path/ovljob.files")  if ($seq eq "parallel");
