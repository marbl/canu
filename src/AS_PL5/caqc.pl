...
MAIN:
{
   ...
   #otherwise, if input already has the ".asm" extension, it will not be touched
    ...
    my @INCLUDE = ("CCO", "UTG", "SCF", "SLK", "AFG", "MDI");
    my $parser = AS::MSG::Parser->open({IN => *IN{IO},
                                        INCLUDE => \@INCLUDE});
#    while ( $record = getCARecord( \*IN ) ) {
    while($record = $parser->next_message()) {
        my $type = $parser->message_type();

        if ( $type eq 'CCO' ) {
            $ncontigs++;
            my $contiglen   = $record->{'len'};
            my $nreads      = $record->{'npc'};
            my $qual        = $record->{'qlt'};
            my $contig_euid = $record->{'acc1'};
            my $contigid    = $record->{'acc2'};

            $sizes{$contig_euid} = $contiglen;
            $lens{$contig_euid}  = $contiglen;
            my $cns = $record->{"cns"};
            my $ngaps    = $cns =~ tr/-//;
            my $n_gcbases = $cns =~ tr/GC//;

            $lens{$contig_euid} -= $ngaps;

            $seqs{$contig_euid} = $nreads;
            $gcs{$contig_euid}  = $n_gcbases;

            foreach my $rec (@{$record->{"VARs"}}) {
                $numVarRecords++;
            }
            $readlens{$contig_euid} = 0;
            foreach my $rec (@{$record->{"MPSs"}}) {
                my $mid  = $rec->{'mid'};
                ...
                if ( exists $bad_mates{$mid} ) {
                    trackBadMates( 'CCO', $contig_euid, $contiglen, $mid,
                                   $rec->{"pos"}{"bgn"}, $rec->{"pos"}{"end"});
                }
            }
            foreach my $rec (@{$record->{"UPSs"}}) {
                my $typ = $rec->{'typ'};
                if ( $typ eq 'S' ) {
                    my ($ub, $ue) = ($rec->{'pos'}{'bgn'}, $rec->{'pos'}{'end'});
            ...
            if ( $record->{'sta'} eq 'U' )    # Daniela Jan 24th 2006
            {
                $uunitigs++;
                $uutgs->add_data( $record->{'len'} );
                $uutgSeqs += $record->{'nfr'};
                $uutglens{$record->{'acc1'}} = $record->{'len'};
            }
            elsif ( $record->{'sta'} ne 'N' ) {

                $nunitigs++;
                my $utgLen = $record->{'len'};
                $utgs->add_data($utgLen);
                $utgSeqs += $record->{'nfr'};
                $utglens{$record->{'acc1'}} = $utgLen;
                $utglensTotal += $utgLen;
                foreach my $rec (@{$record->{"MPSs"}}) {
                    my $mid = $rec->{'mid'};
            ...
            my %scaffcontigs = ();
            my $adj          = 0;
            foreach my $rec (@{$record->{"CTPs"}}) {
                $scaffcontigs{ $rec->{'ct1'} }++;
                $scaffcontigs{ $rec->{'ct2'} }++;
                $gc_scaffcontigs{ $rec->{'ct1'} }++;
                $gc_scaffcontigs{ $rec->{'ct2'} }++;
                $inscaff{ $rec->{'ct1'} } = 1;
                $inscaff{ $rec->{'ct2'} } = 1;
                $adj += $rec->{'mea'};
            }
            ...
            $scaffLinkWeights += $record->{"num"};
        }

        elsif ( $type eq 'AFG' ) {
            my ($frag_id, $fragIID) = ($record->{'acc1'}, $record->{'acc2'});
            my $clrs = $record->{'clr'};
            my ($clrl, $clrr) = ($clrs->{'bgn'}, $clrs->{'end'});
            my $length = $clrr - $clrl;
            $totalCLRReadLengthASM += $length;
            $readLen{$frag_id} = $length;
            $totalSeqs++;

            my $mstField = $record->{'mst'};
            ...
            $numSingletons++ if $record->{'cha'} == 1;
        }

        elsif ( $type eq 'MDI' ) {
            my ($id, $iid) = ($record->{'ref1'}, $record->{'ref2'});
            my $mea  = $record->{'mea'};
            my $std  = $record->{'std'};
            my $min  = $record->{'min'};
            my $max  = $record->{'max'};
    ...
    if ( $frgOpen or $metrics ) {
        ...
        $parser = AS::MSG::Parser->open({IN => *FRG{IO},
                                         INCLUDE => ['DST', 'FRG']});
...
#        while ( $record = getCARecord( \*FRG ) ) {
        while($record = $parser->next_message()) {
            ...
            if ( $type eq 'DST' and $metrics ) {
                ...
                $totalBases += $seqLen;
                my $clr = $record->{'clr'};
                my $clearRangeLen = $clr->{'end'} - $clr->{'bgn'} + 1;
