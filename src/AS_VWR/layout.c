...
static const char *rcsid = "$Id: layout.c,v 1.8 2008-10-08 22:03:02 brianwalenz Exp $";
...
typedef struct LNTag
  { struct LNTag *next;  /* Link for list of all such records */
    ...
    int   *beg;          /* [*beg,*end) is an list of coords with */
    int   *end;          /*   style refs inbetween.               */
    int    attach;       /* Attachment point for links (-1 if none) */
#ifdef DEBUG_ATTACH
    int  ottach;    /* Unadjusted value */
#endif
    int    row;          /* Current row in which line is placed */
    int    extent;       /* Max_L ( end[-1] ) over all L s.t. *(L.beg) < *beg */
...
typedef struct LKTag
  { struct LKTag *next;  /* Link for list of all such records  */
    ...
    int    min;          /* Min attachment coord  */
    int    max;          /* Max attachment coord  */
    int   emin;          /* Min fragment position  */
    int   emax;          /* Max fragment position  */
    int    row;          /* Current row in which group bus bar is placed */
    int    extent;       /* Max_G ( G->max ) over all G s.t. (G.min) < min */
  } Link;
...
static int   MinPnt, MaxPnt; /* Min and Max x-coords in layout */
static int   SmallestLen;    /* Length of shortest piece in layout */
...
{ static long   color;
  ...
  if (FirstCall)
    { FirstCall = 0;
      ...
      xptr = 0;
      intbuffer = (int *) malloc(sizeof(int)*LIST_BUFFER);
...
#define OVLPTYPE 3
    ...
    attlist:
      { char *com;
        ...
        { Style *newStyle;
          newStyle = (Style *) malloc(sizeof(Style));
          newStyle->idnum = idnum;
          newStyle->label = com;
          newStyle->color = color;
          newStyle->style = style;
          newStyle->thick = thick;
          newStyle->usage = 0;
          newStyle->chord = ALLSTYLES;
        ...
        { case ATTRTYPE:
          ...
          case LINKTYPE:
            ...
            { Link *newLink;
              ...
              newLink->beg = linebuffer + lptr;
              while (cnt-- > 0)
                { linebuffer[lptr++] =
                      (Line *) GetIdentifier(LineTable,s,&end,0);
                  s = SpaceAdvance(end);
                }
              newLink->end  = linebuffer + lptr;
              if (att < 0)
                newLink->att = glosty[0];
              else
                newLink->att = att;
              newLink->label = com;
#ifdef DEBUG_LINKIDS
              newLink->idnum = NumLinks;
#endif
          ...
          case OVLPTYPE:
            ...
            { Overlap *newOverlap;
              newOverlap = (Overlap *) malloc(sizeof(Overlap));
              newOverlap->id1 = (Line *) GetIdentifier(LineTable,s,&end,0);
              s = SpaceAdvance(end);
              newOverlap->id2 = (Line *) GetIdentifier(LineTable,s,&end,0);
              if (att < 0)
                newOverlap->att = glosty[0];
              else
                newOverlap->att = att;
              newOverlap->label = com;
              newOverlap->next = OvlpList;
              OvlpList  = newOverlap;
              NumOvlps += 1;
            }
            break;
    ...
    linelist:
      { char *end, *t, *com;
        ...
        { Line *newLine;
          ...
          newLine->attach  = 0;
          newLine->cluster = chk;
          newLine->beg     = intbuffer + xptr;
          newLine->locale  = NumEvents - NumLines;
          ap = num = 0;
          while (cnt-- > 0)
            { lst = num;
              if (*s == 'M')
                num = strtol(s+1,&end,10);
              else
                num = strtol(s,&end,10);
              if (num < 0 ) {
#ifndef TRUNCATE_NEGATIVE_COORDINATES
              ...
              if (cnt > 0)
                if (*s == 'A')
                  { intbuffer[xptr++] = GetIdentifier(StyleTable,s+1,&end,0);
                    s = SpaceAdvance(end);
                  }
                else
                  intbuffer[xptr++] = glosty[(ap++) % glocnt];
            }
          newLine->end   = intbuffer + xptr;
          newLine->label = com;
...
static int AttachMin, AttachSep;

static void MoveLeft(int i, int del)
{ int   play, space;
  Line *ln;
...
static void MoveRight(int i, int del)
{ int   play, space;
  Line *ln;
...
{ Rows = 0;
    ...
    for (i = 0; i < NumLines; i++)
      { ln = LineIndex[i];
        ...
        while (1)
          { last = NULL;
            fn = finger[try];
            while (fn->end[-1] < ln->beg[0]-PADDING)
              { last = finger[try];
                fn = fn->next;
              }
  ...
  if (NumLinks > 0)
    { int i, j, bp, last, max, linkrows, *rowstuff;
...
char *PickAssembly(MT_OBJECT *frame, int x, int y, int v, int mode)
{ int bpzone;
  ...
  { int s, d, i, ans;
    ...
    if (y < 0)
      ...
      { Line *ln;
        ...
        if (v <= 1)
          { if (max < 0)
              sprintf(result,"Mark of %s at %d",ln->idnam,min);
            else if (mode != PICK_PIECES)
              sprintf(result,"Segment %s: [%d,%d] len = %d",
                             ln->idnam,min,max,max-min);
            else
              sprintf(result,"Piece of %s: [%d,%d] len = %d",
                             ln->idnam,min,max,max-min);
            return (result);
          }
        else
...
{ int size, *data, *stack;
  ...
  for (i = 0; i < size; i++)
    { pos = data[i];
      if (pos < 0)
        { pos = -(pos+1);
          while (cnf < val && pos - stack[cnf] > thresh && val > 0)
            data[npt++] = stack[cnf++];
          if (cnf == val&&val>0)
            { data[npt++] = -(pos+1);
              cnf -= 1;
  ...
  data[npt] = 0;
  oprnd->numpts  = npt;
  //maxdep value, set below, is relatively bogus
  oprnd->maxdep  = ndp;
  free(stack);
...
/* Flatten(CoverPacket) reduces coverage to a simple on/off representation */
/* This is not (currently) used, but can be useful for debugging */
static CoverPacket *Flatten(CoverPacket *oprnd)
{ int size, *data;
  int s=0;
  int depth=0;
  int i;
  ...
  /* assumption: first data point is the start of an interval, thus nonnegative */
  assert(data[0]>=0);

  for(i=0;i<size;i++){
    /* if data point is nonnegative, it is the start of a covered segment, so depth of
       covered increases */
    if(data[i]>=0){
      depth++;
    } else {
    ...
    if(depth==0&&data[s-2]==-(data[s-1]+1)){
      //      printf("Retreat!\n");
      s-=2;
    }
  }
...
{ int size, *data,*negdata = NULL;
  ...
  /*
    Check cases:
    1         |----       ----|  Negate takes two intervals to one
    2         |----    ----   |  Negate takes two intervals to two
    3         |   ----    ----|  Negate takes two intervals to two
    4         |  ----    ---- |  Negate takes two intervals to three
    5 	      |---------------|  Negate takes one interval to none
    6	      |               |  Negate takes no intervals to one
  */
  ...
  // Not efficient to do this here, but clear
  { int depth=0;
    int maxdepth=0;
    for(i=0;i<size;i++){
      if(negdata[i]>=0){
...
static HistoPacket *MakeHistogram(int len)
{ HistoPacket *hist;
  ...
  if (len == 0)
    ...
    { int i, sum, *dps;
      ...
      qsort(Histogram,len,sizeof(int),HISTO_COMPARE);
  ...
  *covrR = NULL;
  if (ParseError)
  ...
  else if (NumLines > 0)
    {
      ...
      if (Firstime)
        { Firstime = 0;
          Histogram = (int *) malloc(sizeof(int)*(NumEvents+NumLinks));
        }
  ...
  if (*covrR != NULL)
    DumpCoverage(*covrR);
#else
#ifdef OUTPUT_QUERY_INTERVALS
  printf("QUERY: %s",query);
  if (*covrR != NULL && (*covrR)->numpts>0)
    DumpCoverage(*covrR);
#endif
#endif
...
int DrawHistoInfo(MT_OBJECT *bar, HistoPacket *hist,char *outlabel)
{ char label[100];
  ...
  if (outlabel != NULL ) outlabelptr+=sprintf(outlabelptr,"%*d\t",width,(hist->datapts[0]));
  sprintf(label,"Max = %*d",width,hist->datapts[hist->numpts-1]);
  if (bar != NULL) mt_draw_text(bar,border,23,label);
  if (outlabel != NULL ) outlabelptr+= sprintf(outlabelptr,"%*d\t",width,(hist->datapts[hist->numpts-1]));
  sprintf(label,"Sum = %*d",width,hist->sumofpts);
  if (bar != NULL) mt_draw_text(bar,border,34,label);
  if (outlabel != NULL ) outlabelptr+=sprintf(outlabelptr,"%*d\t",width,(hist->sumofpts));
  ...
  if (bar != NULL) mt_draw_text(bar,border,12,label);
  if (outlabel != NULL ) outlabelptr+=sprintf(outlabelptr,"%*.2f\t",width+3,(1.*hist->sumofpts)/hist->numpts);
  sprintf(label,"Median   = %*d",width,hist->datapts[hist->numpts/2]);
  if (bar != NULL) mt_draw_text(bar,border,34,label);
  if (outlabel != NULL ) outlabelptr+=sprintf(outlabelptr,"%*d\t",width,hist->datapts[hist->numpts/2]);
  sprintf(label,"Std Dev. = %*.2f",width+3,hist->stddev);
  if (bar != NULL) mt_draw_text(bar,border,23,label);
  if (outlabel != NULL ) outlabelptr+=sprintf(outlabelptr,"%*.2f\t",width+3,hist->stddev);
  ...
  if (bar != NULL) mt_draw_text(bar,border,12,label);
  if (outlabel != NULL ) outlabelptr+=sprintf(outlabelptr,"%.2f\t",
          (100.*hist->sumofpts) / (MaxPnt - MinPnt));
  mt_string_size(label,&w,&h,&b);
  max = w;
  sprintf(label,"# of pts.  = %d",hist->numpts);
  if (bar != NULL) mt_draw_text(bar,border,23,label);
  if (outlabel != NULL ) outlabelptr+=sprintf(outlabelptr,"%d",hist->numpts);
  mt_string_size(label,&w,&h,&b);
  if (w > max) max = w;
