...
static const char *rcsid = "$Id: query_celamy.c,v 1.8 2008-10-08 22:03:02 brianwalenz Exp $";
...
{ int e, x, y, v, m;
  ...
  if (e == REDRAW_EVENT)
	...
	if (xe - xb >= 3)
	  { beg = ( xb / MainWidth ) * wid + val;
            end = ( xe / MainWidth ) * wid + val;
            if (beg < 0) beg = 0;
            if (end < 0) end = 0;
          }
        else
...
{ int val0, wid0, min0, max0;
  ...
  min0   += htabexp;
  max0   -= htabexp;
  val1 = min1 + (max1-min1) *  pow( (1.*val1-min1)/(max1-min1) , alpha );
  htabexp = val1*.025;
  min0   -= htabexp;
  max0   += htabexp;
  wid0    = val1 + 2*htabexp;
...
{ MT_OBJECT *obj;
  ...
  { int min, max, rows, smallest;
    ...
    min -= 1;
    wide = (max-min);
// try showing first megabase if otherwise too big
    if ( wide > 1000000) wide = 1000000;
    tab  = wide*.05;
...
{ int val, wid, min, max;
  ...
  zval = zmin + (zmax-zmin) *  pow( (1.*zval-zmin)/(zmax-zmin) , 1./alpha );
...
{ HistoLoad *p;
  ...
  p = (HistoLoad *) d;
  DrawHistoInfoLabel(o,p->hist,label);
  return (0);
}
...
{ MT_OBJECT *obj;
  ...
  textwide = DrawHistoInfoLabel(NULL,hist,label);
  fprintf(stderr,"%s\n",label);
