...
trim_to_pct(vector<MEMatch *>& matches, u32bit midx, float pct) {
  ...
  char *s2 = 0L;
#else
  ArrayAccessor *s2a;		// Can either be forward (normal) or 
                                // ReverseComplement iterator for s2

  // Set up access to sequence 1 in match
  Seq s1 = m->s1();
  s1start = m->pos1();

  // Set up access to sequence 2 in match -- more complex due to
  // possibility of reverse complement
  SeqArrayAccessor s2aa(m->s2());
  ReverseComplement s2rc(s2aa);
  if (m->isReversed()) {
    s2start = m->rcPos2();
    s2a = &s2rc;
  }
  ...
  //  (*s2a) was replaced with s2 below
#endif
...
static void
extend_match_backward(vector<MEMatch *>& matches, u32bit midx, 
  ...
  while ((s1p > min_start_pos) && (s2p > 0)) {

    /* If we've got a match (and N's don't count) ... */
    if ((s1[s1p - 1] != 'N') && (s2[s2p - 1] != 'N') && 
	(s1[s1p - 1] == s2[s2p - 1])) {
      good_run_len++; 
...
can_reach_nearby_match(MEMatch *src, MEMatch *dest)
{
  ...
  // Set up access to sequence 2 in match -- more complex due to
  // possibility of reverse complement
  SeqArrayAccessor s2aa(src->s2());
  ReverseComplement s2rc(s2aa);
  if (src->isReversed()) {
    s2p = src->rcPos2() + src->len();
    s2a = &s2rc;
  }
  else {
    s2p = src->pos2() + src->len();
    s2a = &s2aa;
  }
#endif

  while ((num_mismatch <= DEF_MAX_NBR_PATH_MM) && (s1p < dest->pos1())) {
    if ((s1[s1p] == 'N') || (s2[s2p] == 'N') || (s1[s1p] != s2[s2p]))
      num_mismatch++;
    s1p++;
    s2p++;
  }
...
extend_matches_on_diagonal(vector<MEMatch *>& matches, u32bit diag_start)
{
  ...
  while ((idx < matches.size()) && 
	...
	m->textDump(cout, true, 5);
      ...
      trim_to_pct(matches, idx, DEF_MIN_IDENTITY);
