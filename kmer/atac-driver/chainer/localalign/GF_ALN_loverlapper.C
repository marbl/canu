...
/***********************************************************************
 *
 *             Copyright (c) 2000,2001 Celera Genomics Corporation.
 *                            All rights reserved.
 *
 ***********************************************************************/
	...
	if(O->begpos>=0){
	  ...
	  O->chain[i].agap=MINUS_ONE;
          ...
          if( !(( i==0 && O->chain[i].bgap==O->chain[i].piece.bbpos-1)
            ...
            fprintf(stderr, "PUNT GF_ALN_loverlapper.cc: \n");
            for(int the_piece=0; the_piece < O->num_pieces; the_piece++){
              Local_Segment * const seg =  & (O->chain[the_piece].piece);
              seg->aepos = seg->abpos = 0;
              seg->bepos = seg->bbpos = 0;
            }
            return NULL;
            // return(TraceBuffer);
          }
...
#undef FORCEPOSITIVEBHANG
    ...
    if (computeTraceFlag)/* CMM we do not need the trace computed */ {
      ...
      if(strlen(bseg)!=blen || strlen(aseg)!=alen ) {
        fprintf(stderr,"EXCEPTION strlen(aseg)=%ld alen=%ld abeg=%ld\n", strlen(aseg), alen, abeg);
        fprintf(stderr,"EXCEPTION strlen(bseg)=%ld blen=%ld bbeg=%ld\n", strlen(bseg), blen, bbeg);
        ...
        // return(TraceBuffer); // Return a safe value.
  ...
  if(TraceBuffer != NULL) {
    ...
    if(!(tracep<allocatedspace)){
      /* Really non-robust coding on allocated(trace)space, so let's at least test
         for a violation! */
      // throw(somethingReallyBad);
      fprintf(stderr,"ERROR memory is already corrupted in %s at %s.\n", __FILE__, __LINE__);
      exit(1);
    }
    assert(tracep<allocatedspace); //CMM if this assertion happens, then memory is already corrupted.
  }
...
#define STRETCH (3/2)  /* amount of "stretching" allowed in bounding 
                          overlap end */
