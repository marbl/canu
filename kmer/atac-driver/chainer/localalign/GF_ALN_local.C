...
// CMM undefined REUSE_TABLE_UNSAFE to allow exception handling.
...
#define VARIABLE_SCORE_SCHEME  /* defining this causes scoring to
...
static char *ArevC=NULL,*BrevC=NULL;
...
#undef CHECK_FOR_MISSING_SEGMENT
...
void setup_local_extension_only(int const Alen,int const Blen,
				float const MaxDiff,
				int const MaxiGap)
{ static int   DagMax = -1;
...
int compute_xfactor(int mid){
  return BLOCKCOST+2*mid*diffcost;
}


float  compute_error(Local_Segment *seg){
  return (-seg->score+samecost*(seg->bepos-seg->bbpos))*1./
    (1.*(MATCHCOST)*(seg->bepos-seg->bbpos));
}
