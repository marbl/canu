...
#define REUSE_TABLE_UNSAFE   /* if the user guarantees that this is acceptable, 
...
static void OutOfMemory(char const * const where)
...
Local_Segment *TraceReversePath
(char const * const A, int const Alen,
 char const * const B, int const Blen,
...
static Trapezoid *Build_Trapezoids
(char const * const A, int const Alen,
 char const * const B, int const Blen,
...
(char const * const A, int const Alen,
 ...
 int const MinLen, float const MaxDiff, int const Traplen)
{ int j, mid, indel;
 float pcnt;
...
static Local_Segment *Align_Trapezoids
(char const * const A, int const Alen,
 char const * const B, int const Blen,
 Trapezoid const * const Traplist, int const Traplen,
 int const start, int const comp,
 int const MinLen, float const MaxDiff, int * const Seglen)
{ static int fseg;
 static int TarMax = -1;
...
Local_Segment *Find_Local_Segments(char const * const A, int const Alen,
  ...
  /* defining this causes scoring to be set so that nearly all
   extensions terminate within the user-defined error rate; this means
   we don't completely miss a high-quality segment that is part of a
   larger segment of lower quality--e.g., we could find more-conserved
   portions of a repeat even if the repeat as a whole was lower
   fidelity, due to varying selectional pressure at different
   positions; the down-side to this is that we won't chain together
   perfect matches across a few bases of lower-fidelity sequence.
   Leaving it undefined uses the #define values of DIFFCOST and
   SAMECOST */
  samecost=(int)ceil(100.*MaxDiff);
