...
encodedQuery::getMer(u64bit &mer, u32bit &pos) {
  ...
  if (_rc) {
    ...
    while (!found && (_seqPos < _seqLen)) {
      ...
      if (validSymbol[(int)_seq[_seqLen - 1 - _seqPos]]) {
        _substring |= compressSymbol[ complementSymbol[ (int)_seq[_seqLen - 1 - _seqPos] ]];
        _timeUntilValid--;
      } else {
        _timeUntilValid = _merSize;
      }
    ...
    while (!found && (_seqPos < _seqLen)) {
      ...
      if (validSymbol[(int)_seq[_seqPos]]) {
        _substring |= compressSymbol[ (int)_seq[_seqPos] ];
        _timeUntilValid--;
      } else {
        _timeUntilValid = _merSize;
      }
